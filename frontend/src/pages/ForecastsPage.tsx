...
export function ForecastsPage() {
  ...
  return (
    <>
      <PageIntro kicker="Forecasting" title="Short-term forecasts and long-range scenarios" description="The 1 to 6 month model is fitted to observed price series. The 12 to 24 month scenario uses DDR5 momentum, official semiconductor indicators, and attributed industry research." />
      <DataBoundary loading={state.loading || prices.loading || analytics.loading} error={state.error || prices.error || analytics.error}>
        <section className="industry-outlook-hero">
          <div className="industry-outlook-hero__copy">
            <p className="kicker">Published industry outlook through 2027</p>
            <span className="outlook-direction"><i />Upward DRAM pressure</span>
            <h2>Published DRAM outlooks point upward.</h2>
        ...
        {evidence ? <section className="forecast-evidence-readiness" aria-label="DDR5 evidence readiness">
          <div className="forecast-evidence-readiness__intro">
            <div><p className="kicker">Evidence readiness</p><h2>{evidence.label}</h2><p>{evidence.explanation}</p></div>
        ...
        {structural.length ? <section className="structural-forecast-section">
          <div className="section-heading"><div><p className="kicker">Market scenario · 12 to 24 months</p><h2>A driver-based range for the next two years</h2></div><p>The scenario combines clipped DDR5 momentum, official semiconductor prices and capacity, plus attributed industry research. Easing, base, and tight-supply cases show the assumption range.</p></div>
        ...
        <div className="forecast-scope-divider"><span>Series model · 1 to 6 months</span><div><h2>Select a public price series</h2><p>This model is descriptive and backtested. A flat midpoint means that no trend model beat the naive baseline for the selected series. It does not represent the broader industry outlook.</p></div></div>
        {forecast ? (
          <>
            ...
            <section className="chart-card forecast-history"><div className="section-heading"><div><p className="kicker">Selected short-term forecast · {formatDate(forecast.target_date)}</p><h2>{formatNumber(forecast.point_forecast, 3)} <small>source-defined units</small></h2></div><p>95% interval {formatNumber(forecast.lower_bound, 3)} to {formatNumber(forecast.upper_bound, 3)}. The chart uses the latest complete forecast vintage.</p></div><ForecastFanChart history={history} forecasts={activeForecasts} /><p className="chart-takeaway"><strong>Analyst note</strong>{flatBaseline ? `The midpoint stays flat because no trend model beat the naive baseline for ${activeSeries}. This local baseline is separate from the broader industry outlook.` : forecastChange == null ? 'The model has published a direction, but no comparable latest value is available for a percentage change.' : `The selected estimate is ${Math.abs(forecastChange).toFixed(1)}% ${forecastChange >= 0 ? 'above' : 'below'} the latest observed value. Evaluate the estimate using its full interval.`}</p>{history && <p className="chart-summary">{history.source_label} · {history.basis}</p>}</section>
